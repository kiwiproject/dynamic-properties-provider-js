...
function getPropertyListForObject(obj: any): Array<Property> {
  if (!obj.getDynamicProperties) {
    throw new Error('The given object does not have a "getDynamicProperties" method. Can not process property list.');
  }
 ...
 */
function setupDynamicPropertiesEndpoints(objects: Record<string, any>): express.Router {
  ...
  router.get("/kiwi/dynamic-properties/:identifier", (req, res) => {
    const objectProps = getPropertyListForObject(objects[req.params.identifier]);
